...
bike_orderlines_joined_tbl$location

bike_orderlines_joined_tbl %>% 
  select(location) %>%
  filter(str_detect(location, "Bavaria")) %>% 
  unique()

# All actions are chained with the pipe already. You can perform each step separately and use glimpse() or View() to validate your code. Store the result in a variable at the end of the steps.
bike_orderlines_wrangled_statecity_tbl <- bike_orderlines_joined_tbl %>%
...
library(lubridate)
# Step 1 - Manipulate
sales_by_state_tbl <- bike_orderlines_wrangled_statecity_tbl %>%
...
# Step 1 - Manipulate
sales_by_year_state_1_tbl <- bike_orderlines_wrangled_statecity_tbl %>%
  
  # Select columns and add a year
  select(order_date, total_price, state) %>%
  mutate(year = year(order_date)) %>%
  
  # Group by and summarize year and main categegory
    group_by(year, state) %>%
